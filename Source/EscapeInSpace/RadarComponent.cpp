...
URadarComponent::GetPointsOnPlane()
{
	...
	for (auto& a : trackedObjects)
	{
		...
		if (actorToTrackedObjectReal.Size() < MaxRadarDistance)
		{
			...
			RealDistanceToTrackerObjects.Add(actorToTrackedObjectReal.Size() / MaxRadarDistance);
		}
	}
...
URadarComponent::UpdateTrackedObjects()
{
	...
	RealDistanceToTrackerObjects.Empty();
...
float 
URadarComponent::GetRealDistanceToTrackerObjects(int32 index)
{
	return RealDistanceToTrackerObjects[index];
}
